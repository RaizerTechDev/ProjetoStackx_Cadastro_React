...
export function Home() {
  ...
  const filteredCountry =
    searchCountry.length > 0
      ? dataFetching.filter((a) =>
          a.location.country.toUpperCase().includes(searchCountry.toUpperCase())
        )
      : [];
  ...
  const filteredName =
    search.length > 0
      ? dataFetching.filter((a) =>
          (a.name.first.toUpperCase() || a.name.last.toUpperCase()).includes(
            search.toUpperCase()
          )
        )
      : [];
  ...
  return (
    <Container>
      <Header />
      <Content>
        <h1>Lista dos Cadastros Clientes</h1>
        {isFetching ? (
                  ...
                  ? actuallyFilter.map((e, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <td>
                            <button onClick={() => handleOpenModalInfo(e)}>
                              Visualizar
                            </button>
                          </td>
                        </tr>
